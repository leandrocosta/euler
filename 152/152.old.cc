...
const unsigned int CACHE_ITEM_BITS = 8 + 20; //10; //22; //20; //10; //3;
const unsigned int CACHE_SIZE = 256 * 1024 * 1024; //4*1024*1024; //1024; //8;
...
void check_cache (double total, TermsList& solution)
{
	...
	*/
	...
	if (CACHE_SIZE > 0 && FLOAT_GE(d, cache[min]) && FLOAT_LE(d, cache[max]))
	{
		...
		while (FLOAT_EQ (total + cache[mid], 0.50))
		{
...
void run (double total, unsigned int i, TermsList& solution)
{
	...
	if (FLOAT_LT(total, 0.5))
	{
		for (++i; (i <= MAX_VALUE - CACHE_ITEM_BITS) && (FLOAT_GE(total + max_possible_total[i], 0.5)); i++)
		{
	...
	else if (total < 0.5)
	{
		if (i < MAX_VALUE - CACHE_ITEM_BITS)
		{
			...
			{
				if ((total + max_possible_total[i]) >= 0.5)
				{
					run (total, i, solution);
				}
			}
